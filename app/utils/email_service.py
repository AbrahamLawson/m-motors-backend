...
conf = ConnectionConfig(
    MAIL_USERNAME="groupe5sitevehicule@gmail.com",
    MAIL_PASSWORD="Azerty12345@",
    MAIL_FROM="groupe5sitevehicule@gmail.com",
    MAIL_PORT=587,
...
def get_email_content(reservation_type: str, status: str, details: Dict) -> tuple:
    ...
    if not template:
        raise ValueError(f"Pas de template trouvé pour le type '{reservation_type}' et le statut '{status}'")
    return template["subject"], template["body"]

async def send_reservation_notification(email: str, reservation_type: str, status: str, reservation_details: dict):
    # Ne pas envoyer d'email pour le statut 'in_progress'
